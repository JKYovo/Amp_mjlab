...
class V4Test(unittest.TestCase):
    ...
    def test_config_and_fresh_defaults(self):
        ...
        for play in (False, True):
            ...
            self.assertEqual(command.ranges.lin_vel_y, (-0.5, 0.5))
            self.assertEqual(command.ranges.ang_vel_z, (-1.57, 1.57))
            self.assertEqual(command.ranges.heading, (-math.pi, math.pi))
            self.assertEqual(command.turning_max_abs_ang_vel, 1.57)
